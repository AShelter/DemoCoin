...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("SukaCoin"))
        return false;
...
{
    // Convert SukaCoin:// to SukaCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("SukaCoin://"))
    {
        uri.replace(0, 12, "SukaCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SukaCoin.lnk";
}
...
{
    return GetAutostartDir() / "SukaCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SukaCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  SukaCoin-qt [" + tr("command-line options") + "]                     " + "\n";
